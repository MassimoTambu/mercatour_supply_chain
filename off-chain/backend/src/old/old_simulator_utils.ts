...
class _SU {
  ...
  static getFundWallet(): SupplyChainWallet {
    const seedPhrase = _SU.getEnvVar("FUND_WALLET_SEED_PHRASE");
    const verificationKey = _SU.getEnvVar("FUND_VERIFICATION_KEY");
    const verificationKeyHash = _SU.getEnvVar("FUND_VERIFICATION_KEY_HASH");
    const wallet = walletFromSeed(seedPhrase, { network: 'Preview' });
    return {
      ...wallet,
      seedPhrase,
  ...
  */
  static async createUserNFTCertificates(lucid: LucidEvolution, receiverAddresses: Address[]): Promise<string> {
    const fundWallet = _SU.getFundWallet();
    const tokenName = _SU.getEnvVar("USER_NFT_CERTIFICATE_TOKEN_NAME");
    const mintCompiledCode = _SU.getPlutusUserNFTMintCompiledCode();
    ...
    const datum = _SU.generateCIP68Metadata();
  ...
  private static generateCIP68Metadata(): string {
    const name = _SU.getEnvVar("USER_NFT_CERTIFICATE_NAME");
    const description = _SU.getEnvVar("USER_NFT_CERTIFICATE_DESCRIPTION");
    const image = _SU.getEnvVar("USER_NFT_CERTIFICATE_IMAGE");
    const expiration = _SU.getEnvVar("USER_NFT_CERTIFICATE_EXPIRATION");
  ...
  static async registerProducts(lucid: LucidEvolution, wallets: SupplyChainWallet[]): Promise<string[]> {
    ...
    const mintCompiledCode = _SU.getRegisteredProductsMintCompiledCode();
