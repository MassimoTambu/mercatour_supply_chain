...
export class SU {
  ...
  static async mintProducts(lucid: LucidEvolution, productActions: ProductAction[]): Promise<string[]> {
    ...
    for (const productAction of productActions) {
      const wallet = productAction.wallet;
      const productTransactionMetadata = productAction.product;
      const userTokenQuantity = productAction.quantity;
      if (userTokenQuantity === undefined || userTokenQuantity === null) {
        console.error(`Invalid quantity: ${userTokenQuantity}. It must have a value.`);
        continue;
      }
      ...
      console.log(`Product minted with tx hash: ${txHash}`);
  ...
  static async reMintProducts(lucid: LucidEvolution, productActions: ProductAction[]): Promise<string[]> {
    const tokenName = SU.getEnvVar("NFT_ASSET_NAME");
    const mintCompiledCode = SU.getRegisteredProductsMintCompiledCode();
    const script = applyDoubleCborEncoding(mintCompiledCode);
    const spendingValidator: SpendingValidator = { type: "PlutusV3", script };
    ...
    for (const productAction of productActions) {
      ...
      const signedTx = await txToSign.sign.withWallet().complete();
      const txHash = await signedTx.submit();
      console.log(`Product reminted with tx hash: ${txHash}`);
      txHashes.push(txHash);
    }
  ...
  static async burnProducts(lucid: LucidEvolution, productActions: ProductAction[]): Promise<string[]> {
    const tokenName = SU.getEnvVar("NFT_ASSET_NAME");
    const mintCompiledCode = SU.getRegisteredProductsMintCompiledCode();
    const script = applyDoubleCborEncoding(mintCompiledCode);
    const spendingValidator: SpendingValidator = { type: "PlutusV3", script };
    ...
    for (const productAction of productActions) {
      ...
      if (userUnitAsset < BigInt(userTokenQuantity)) {
        console.error(`Not enough user unit assets to burn: ${userUnitAsset} at address: ${wallet.address}`);
        continue;
      }

      // Get the reference NFT in the locked script address of the validator
      // * Not really needed, but it is a good practice to check if the reference NFT exists
      const validatorUtxo = (await lucid.utxosAt(validatorAddress))
        .find((utxo) => utxo.assets[refUnit] &&
          utxo.assets[refUnit] === 1n &&
          utxo.datum === datum);
  ...
  static async burnAllProducts(lucid: LucidEvolution, productActions: ProductAction[]): Promise<string[]> {
    const tokenName = SU.getEnvVar("NFT_ASSET_NAME");
    const mintCompiledCode = SU.getRegisteredProductsMintCompiledCode();
    const script = applyDoubleCborEncoding(mintCompiledCode);
    const spendingValidator: SpendingValidator = { type: "PlutusV3", script };
